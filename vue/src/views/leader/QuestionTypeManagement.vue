<template>
    <div class="page-container">
        <div class="page-header">
            ...
            <el-button type="success" @click="handleAdd" :icon="Plus">新增类型</el-button>
        </div>

        <el-card class="table-card" shadow="never">
            <div class="search-bar">
                <el-input v-model="searchName" placeholder="搜索类型名称..." clearable style="width: 200px" />
        ...
        <el-dialog v-model="dialogVisible" :title="form.id ? '编辑类型' : '新增类型'" width="450px">
            ...
            <template #footer>
                ...
                <el-button type="success" @click="save">保存并应用</el-button>
            </template>
        </el-dialog>
    </div>
</template>
...
const load = async () => {
    loading.value = true
    try {
        const res = await request.get('/question-type/list', { params: { name: searchName.value || undefined } }) as any
...
const handleDelete = (row: any) => {
    ElMessageBox.confirm('确定删除该题目类型？', '提示', { type: 'warning' }).then(async () => {
        await request.delete('/question-type/' + row.id)
        ElMessage.success('已删除'); load()
    })
}
const save = async () => {
    if (form.id) { await request.put('/question-type', form) } else { await request.post('/question-type', form) }
